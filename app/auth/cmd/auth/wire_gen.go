...
func wireApp(confServer *conf.Server, confData *conf.Data, auth *conf.Auth, logger *slog.Logger) (*kratos.App, func(), error) {
	...
	emailVerificationConfig := NewEmailVerificationConfig(auth)
	emailVerificationUsecase := biz.NewEmailVerificationUsecase(emailVerificationRepo, userRepo, tokenUsecase, publisher, emailVerificationConfig)
	string2 := NewFrontendURL(auth)
	authUsecase := biz.NewAuthUsecase(userRepo, sessionRepo, passwordResetTokenRepo, tokenUsecase, auditUsecase, publisher, cache, cache, cache, emailVerificationUsecase, string2)
	authService := service.NewAuthService(authUsecase, tokenUsecase)
	magicLinkRepo := data.NewMagicLinkRepo(dataData)
	magicLinkUsecase := biz.NewMagicLinkUsecase(magicLinkRepo, userRepo, sessionRepo, tokenUsecase, publisher, string2)
	...
	socialConnectionRepo := data.NewSocialConnectionRepo(dataData)
	socialUsecase := biz.NewSocialUsecase(userRepo, socialConnectionRepo, sessionRepo, tokenUsecase, publisher)
	oAuthStateRepo := data.NewOAuthStateRepo(cache)
	socialService := service.NewSocialService(socialUsecase, oAuthStateRepo)
	emailVerificationService := service.NewEmailVerificationService(emailVerificationUsecase, tokenUsecase)
	httpServer := server.NewHTTPServer(confServer, auth, authService, magicLinkService, mfaService, adminService, socialService, emailVerificationService)
